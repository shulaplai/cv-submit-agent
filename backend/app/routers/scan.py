...
async def _scan_job(track: str | None = None):
    db: Session = SessionLocal()
    try:
        progress = _state["progress"]
        progress.update({"platform": "", "phase": "", "count": 0})
        summary: ScanSummary = await run_scan(db, progress, track=track)
        _state["last"] = {
...
async def start_scan(payload: dict | None = None):
    ...
    track = (payload or {}).get("track", "all")
    ...
    _state["stop_requested"] = False
    _state["track"] = track
    asyncio.create_task(_scan_job(track))
    label = {"all": "全部職位", "it": "IT 職位", "general": "一般職位"}[track]
    return {"started": True, "message": f"scan 已開始（{label}），請稍後查詢狀態"}
